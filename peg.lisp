...
(ESRAP:DEFRULE GRAMMAR (AND SPACING (esrap:+ DEFINITION) SPACING ENDOFFILE)
  (:DESTRUCTURE
   (SPC DEF SPC2 EOF)
...
(ESRAP:DEFRULE SEMANTICCODE (AND OPENBRACE (esrap:+ NOTBRACE) CLOSEBRACE)
  (:DESTRUCTURE
   (LB CODE RB)
...
(ESRAP:DEFRULE EXPRESSION (AND PSEQUENCE (esrap:* SLASHSEQUENCE))
...
(ESRAP:DEFRULE PSEQUENCE (esrap:* PREFIX)
  (:DESTRUCTURE
   (&REST PREF)
   (IF PREF
...
(ESRAP:DEFRULE STRINGIDENTIFIER (AND IDENTSTART (esrap:* IDENTCONT) SPACING)
  (:TEXT T))
...
(ESRAP:DEFRULE PCLASS (AND "[" (esrap:* NOTRB) "]" SPACING)
  (:DESTRUCTURE
   (LB RANGE RB SPC)
...
(ESRAP:DEFRULE SPACING (esrap:* (OR PSPACE COMMENT))
  (:LAMBDA (LIST) (DECLARE (IGNORE LIST)) (VALUES)))

(ESRAP:DEFRULE COMMENT
               (AND "#"
                    (esrap:* (AND (ESRAP:! ENDOFLINE) CHAR1))
